...
export const handlers: ToolHandlerMap = {
    ...
    async search_customers_by_email(args) {
          ...
          // Shopmonkey's /customer/email/search endpoint requires an `emails` array, not a singular `email` field
      // (confirmed against the live API: a singular field is rejected with "body must have required property 'emails'").
