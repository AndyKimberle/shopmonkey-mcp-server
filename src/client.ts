...
const DEFAULT_MAX_PAGES = 80; // 80 * 100 = 8000 records — see fetchAllRecordsParallel below.

/**
 * fetchAllRecords (above) pages sequentially and stops once a shop's total
 * record count exceeds `maxRecords` — verified live on this shop's /order
 * data (2000+ orders): capping at 500, then 1500, still produced a
 * different subset of orders on every identical call, because the
 * underlying list has no stable sort and the capped window only ever
 * covered a fraction of the total. Three consecutive identical revenue
 * queries returned three different totals as a direct result.
 *
 * fetchAllRecordsParallel takes a different approach for endpoints with
 * large record counts: instead of stopping at a cap, it requests a
 * generous, fixed number of pages (maxPages) *concurrently* — bounded by
 * the same MAX_CONCURRENT request-slot limiter every other call already
 * goes through — and simply keeps whatever came back. Because every call
 * requests the same full set of pages regardless of how many records
 * actually exist, the result no longer depends on where an arbitrary
 * cutoff happened to land, which is what caused the inconsistency above.
 * `truncated` is only set if the very last page in the budget came back
 * completely full, meaning there could be more records beyond it.
 *
 * This trades some wasted requests (pages fetched past the real end of
 * the data return empty and are simply dropped) for correctness and
 * determinism, and finishes in roughly maxPages / MAX_CONCURRENT sequential
 * round-trips instead of maxPages sequential ones.
 */
export async function fetchAllRecordsParallel<T extends { id?: unknown }>(
                    path: string,
                    params?: Record<string, string>,
  ...
  const pagePromises: Array<Promise<T[]>> = [];
                    for (let i = 0; i < maxPages; i++) {
                                                const skip = i * pageSize;
                                                pagePromises.push(
                                                                                      shopmonkeyRequest<T[]>('GET', path, undefined, {
                                                                                                                                        ...params,
  ...
  // Let a genuine failure (auth error, network error, etc.) propagate and
  // fail the whole call rather than silently treating a failed page as
  // "no more records" — a quietly wrong number is worse than a clear error.
  ...
  const records: T[] = [];
      ...
      const lastPage = pages[pages.length - 1];
                    const truncated = Array.isArray(lastPage) && lastPage.length >= pageSize;
